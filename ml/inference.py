...
class OrientationModel:
    ...
    def _reason_phrase(self, profile: dict, domaine_id: str) -> str:
        ...
        sentence fragment instead of a number. Always given, for every
        domain returned by rank_domaines/score_adequation: the assistant
        should always say what brought a profile close to a domain (e.g.
        "tu as mentionné : mathématiques"), never hedge with an abstract
        confidence/uncertainty label instead of a concrete reason."""
    ...
    def rank_domaines(self, profile: dict, k: int = 3) -> list[dict]:
        ...
        adequacy, each with a natural-language reason grounded in the
        declared features, and the looked-up candidate ISPM filières. This
        is `classer_parcours` / `analyser_profil` from the brief's example
        tool list, generalised to a domain rather than a single ISPM
        filière.
